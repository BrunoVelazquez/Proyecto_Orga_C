    ...
    if( valor_utilidad(estadoN, es_max)== (IA_NO_TERMINO)){ // Esta bien
            ...
            if(es_max){
                ...
                while(cursor<=longitud && corte==0){
                    ...
                    if(mejor_valor_sucesores<(estado_nuevo_nodo->utilidad)) mejor_valor_sucesores=estado_nuevo_nodo->utilidad;

                    if(alpha<mejor_valor_sucesores){

                            alpha= mejor_valor_sucesores;
                    }
                    ...
                    while(cursor<=longitud && corte==0){
                        ...
                        if(mejor_valor_sucesores> (estado_nuevo_nodo->utilidad)) {
                            mejor_valor_sucesores=estado_nuevo_nodo->utilidad;

                        }

                        if(beta>mejor_valor_sucesores) beta= mejor_valor_sucesores;
...
static int valor_utilidad(tEstado e, int jugador_max){
    ...
    int no_encontre_resultado=1;
     ...
     if(( e->grilla[0][0])!=PART_SIN_MOVIMIENTO){
          ...
          if ((( e->grilla[0][0]) == ( e->grilla[0][1]) == (e->grilla[0][2])) || (( e->grilla[0][0]) == ( e->grilla[1][0]) == ( e->grilla[2][0])) || (( e->grilla[0][0]) == ( e->grilla[1][1]) == ( e->grilla[2][2]))){
     ...
     if((e->grilla[1][1])!=PART_SIN_MOVIMIENTO){
            ...
            if (((e->grilla[1][0]) == (e->grilla[1][1]) == (e->grilla[1][2])) || ((e->grilla[0][1]) == (e->grilla[1][1]) == (e->grilla[2][1])) || ((e->grilla[2][0]) ==(e->grilla[1][1]) == (e->grilla[0][2]))) {
     ...
     if((e->grilla[2][2])!=PART_SIN_MOVIMIENTO){
        ...
        if (((e->grilla[2][0]) == (e->grilla[2][1]) == (e->grilla[2][2])) || ((e->grilla[0][2]) == (e->grilla[1][2]) == (e->grilla[2][2]))){
        ...
        if(no_encontre_resultado){
            ...
            while(i<2 && no_encontre_resultado==0){
                while(j<2 && no_encontre_resultado==0){
                     if (e->grilla[i][j]!= PART_SIN_MOVIMIENTO) j=j+1;
                        else no_encontre_resultado=1; //quiere decir que IA NO TERMINO
                }
                i=i+1;
            }

            if(no_encontre_resultado==1) ret=IA_NO_TERMINO;
    }
    return ret;
}
